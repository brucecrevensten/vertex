...
var LegacyFacetDialog = PlatformFacetView.extend( {
    ...
    setFilters: function() {
        ...
        // Flight Directions
        $(this.el).find('input[name="direction"]')
        .click(jQuery.proxy(function(e) {
            $('.ui-dialog-buttonpane').find('button:contains("Apply")').button().focus();
            var curEl = $(e.currentTarget);
        ...
        //  Path
        $(this.el).find('input[name="path"]')
        .bind('input', jQuery.proxy(function(e) {
            $('.ui-dialog-buttonpane').find('button:contains("Apply")')
            .attr("class", 'ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only ui-state-hover');
